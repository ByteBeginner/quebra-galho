...
export const create = async (req, res) => {
  try {
    ...
    switch (true) {
      ...
      case !shipping.trim():
        return res.json({ error: "Método de envio é obrigatório" });
      case photo && photo.size > 1000000:
        return res.json({ error: "A imagem deve ter tamanho menor que 1MB" });
    }
...
export const filteredProducts = async (req, res) => {
  try {
    ...
    if (radio.length) args.price = { $gte: parseInt(radio[0]), $lte: parseInt(radio[1]) };
