...
export default function AdminCategory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.error) {
        ...
        loadCategories();
        setName("");
        toast.success(`"${data.name}" criada com sucesso!`);
      }
    } catch (err) {
      console.log(err);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.put(`${process.env.REACT_APP_API}/category/${selected._id}`, {
        name: updatingName,
      });
      if (data?.error) {
        toast.error(data.error);
      } else {
        toast.success(`"${data.name}" atualizada`);
        setSelected(null);
        setUpdatingName("");
        loadCategories();
        setVisible(false);
  ...
  const handleDelete = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.delete(`${process.env.REACT_APP_API}/category/${selected._id}`);
      if (data?.error) {
        toast.error(data.error);
      } else {
        toast.success(`"${data.name}" deletada`);
        setSelected(null);
        loadCategories();
        setVisible(false);
      }
  ...
  return (
    <>
      <Jumbotron
        ...
        subTitle="Área de Administração"
      />
