  ...
  const loadRelated = async (productId, categoryId) => {
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_API}/related-products/${productId}/${categoryId}`
      );
      setRelated(data);
    } catch (err) {
      console.log(err);
      ...
      <div className="row">
        <div className="col-md-9">
          <div className="card mb-3">
            <Badge.Ribbon text={`${product?.sold} vendido`} color="red">
              <Badge.Ribbon
                text={`${
                  product?.quantity >= 1
                    ? `${product?.quantity - product?.sold} no estoque`
                    : "Fora de estoque"
                }`}
                placement="start"
                color="green"
        ...
        <div className="col-md-3">
          ...
          <hr />
          {related?.length < 1 && <p>Não encontrado</p>}
          {related?.map((p) => (
